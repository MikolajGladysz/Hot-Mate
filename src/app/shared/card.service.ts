...
swipe:number
  ...
  getCurrentUser():EventEmitter<User>{
    return this.currUser;
  }
  setCurrentUser(user:User){
    this.currUser.emit(user);
  }
  emitUsersObs(){
    this.usersObs.emit()
  }
  newMatch(user:User){
    if(user){
      setTimeout(()=>{this.newMatchObs.emit(user);},1000)
    }else{
  ...
  generateUsers(num:number,newUser?:boolean){
    ...
    for(let i=0;i<num;i++){
      const random = (min:number,max:number)=>Math.trunc(Math.random()*max)+min;
      const tags = ['Quick chess','Bullet','Classic','E4 Enjoyer,','Good tactic','Outdor chess','Chess online','Tournament Player','Begginer']
      const names = ['Anna','Nikola','Kundegunda','Basia','Anastazja','Wiola','Bogumiła','Danusia','Jolanta','Ola','Adam','Jan','Tomasz','Mikołaj','Krzystof','Piorek','Paweł']
      const lorem = 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Corporis inventore blanditiis esse omnis earum similique deleniti quod incidunt quibusdam facere, rerum magni quaerat odit enim dolorem porro sequi, est odio?'.split(' ')
      const openings = ['Karo Kann','Gambit Sandomierski','Bong Cloud','Gamit Królowej','Alfabetycznie','Gamit Króla']
      const cities = ['Świdwin',"Rumia","Toront",'Warszawa',"Poznan","Kraków","Zielona Góra",'London','New York']
      const chessTitles = ["Grand Master","International Master","Country Master","Master Wsi","",""]
      const photos = ["../../assets/images/user1/360_F_224869519_aRaeLneqALfPNBzg0xxMZXghtvBXkfIA.jpg"
    ...
    let messangeArr:User['messanges'] = [];
    if(!newUser){
      Array(random(1,8)).fill('').forEach(()=>{
        messangeArr.push({fromUser:random(0,2)?true:false,messange:{date:'10.11.22',content: lorem.slice(0,random(3,lorem.length-1)).join(' ')} })
      })
    }else{
    ...
    const favOpening = { openingName:openings[random(0,openings.length-1)],opening:'laldaj'}
    const favGames = new Set()
    Array(random(1,10)).fill('').forEach(()=>{
    ...
    if(newUser){
      return new User(name,age,photoArr,distance,description,tagArr,favOpening,favGamesArr,city,chessTitle,random(0,2)?messangeArr:undefined)
    }
    this.users.push(new User(
      name,age,photoArr,distance,description,tagArr,favOpening,favGamesArr,city,chessTitle,random(0,2)?messangeArr:undefined
    ))
